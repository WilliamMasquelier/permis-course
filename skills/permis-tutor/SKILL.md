...
- `${CLAUDE_SKILL_DIR}/prompts/tutor-system-prompt.md` — RTRI system prompt template.
...
- Read `${CLAUDE_SKILL_DIR}/prompts/tutor-system-prompt.md`.
...
This skill is part of the `permis-course` plugin. All file paths (`Wiki/`, `output/`, `scripts/`) are relative to the plugin root. When running shell commands, use `cd "${CLAUDE_SKILL_DIR}/../.." &&` as a prefix (e.g. `cd "${CLAUDE_SKILL_DIR}/../.." && uv run python scripts/render_course.py`).
