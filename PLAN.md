...
For *Permis Bateau*: `~/Documents/Permis Bateau/` with `Cours 1/`,
`Cours 2/`, plus loose admin PDFs (~70 PDFs + 139 MP3 oral drills + 1
PowerPoint).
...
- **Audio drills** (already present for *Permis Bateau*) — Whisper-
  transcribe `Cours 2/*.mp3` → `raw/audio-transcripts/` → ingest into
  `wiki/questions/`.
...
1. Continue the ingest pass on the 8 RECTO/VERSO summary cards in
   `Cours 2/` — they have the highest signal-to-noise.
2. Whisper-transcribe `Cours 2/*.mp3` (139 files) into `raw/audio-transcripts/`.
